...
def validate_recipients(memo_data: MemoCreate | MemoUpdate):
    ...
    if len(memo_data.title.strip()) > 100:
        raise HTTPException(status_code=400, detail="Memo title must be 100 characters or fewer.")
    if not memo_data.content.strip():
        raise HTTPException(status_code=400, detail="Memo content is required")
    if len(memo_data.content.strip()) > 500:
        raise HTTPException(status_code=400, detail="Memo content must be 500 characters or fewer.")
