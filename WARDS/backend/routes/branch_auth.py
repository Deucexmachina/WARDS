...
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIRE_MINUTES = 480  # 8 hours for branch staff
...
async def request_otp(request: Request, credentials: OTPRequest, db: Session = Depends(get_db)):
    ...
    if not check_rate_limit(client_ip):
        log_activity(db, "OTP Request Rate Limited", credentials.username, f"IP: {client_ip}", "branch_auth")
        raise HTTPException(
    ...
    if is_account_locked(credentials.username):
        log_activity(db, "OTP Request on Locked Account", credentials.username, f"IP: {client_ip}", "branch_auth")
        raise HTTPException(
    ...
    if not password_valid:
        record_failed_attempt(credentials.username)
        log_activity(db, "Failed OTP Request", credentials.username, f"Invalid password from IP: {client_ip}", "branch_auth")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid username or password"
        )
...
async def branch_login(request: Request, credentials: BranchLoginRequest, db: Session = Depends(get_db)):
    ...
    if not check_rate_limit(client_ip):
        log_activity(db, "Login Rate Limited", credentials.username, f"IP: {client_ip}", "branch_auth")
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
    ...
    if is_account_locked(credentials.username):
        ...
        log_activity(db, "Login Attempt on Locked Account", credentials.username, f"IP: {client_ip}", "branch_auth")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    if not staff or not pwd_context.verify(credentials.password, staff.hashed_password):
        ...
        log_activity(db, "Failed Login Attempt", credentials.username, f"Invalid credentials from IP: {client_ip}, Attempts left: {attempts_left}", "branch_auth")
    ...
    if staff.status != "Active":
        log_activity(db, "Login Attempt for Inactive Account", credentials.username, f"Status: {staff.status}, IP: {client_ip}", "branch_auth")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Account is not active"
        )
    ...
    if not credentials.otp:
        log_activity(db, "Login Without OTP", credentials.username, f"IP: {client_ip}", "branch_auth")
        raise HTTPException(
    ...
    if credentials.username not in otp_storage:
        log_activity(db, "Login with Invalid OTP Session", credentials.username, f"IP: {client_ip}", "branch_auth")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if time.time() > otp_data["expires_at"]:
        ...
        log_activity(db, "Login with Expired OTP", credentials.username, f"IP: {client_ip}", "branch_auth")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if credentials.otp != otp_data["otp"]:
        record_failed_attempt(credentials.username)
        log_activity(db, "Login with Incorrect OTP", credentials.username, f"IP: {client_ip}", "branch_auth")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid OTP"
        )
...
async def branch_logout(request: Request, db: Session = Depends(get_db)):
    ...
    if auth_header and auth_header.startswith("Bearer "):
        ...
        try:
            ...
            log_activity(db, "Branch Logout", username or "Unknown", f"IP: {client_ip}", "branch_auth")
        except:
            pass
