...
import api, { branchAnnouncementAPI } from '../../services/api';
...
const BranchAnnouncements = () => {
  ...
  const handleViewAnnouncement = async (announcement) => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('branch-announcement-viewed', { detail: { unreadCount } }));
    } catch (error) {
  ...
  const handleSaveAnnouncement = async () => {
    ...
    try {
      ...
      if (editingAnnouncement) {
        const response = await api.put(`/branch/announcements/${editingAnnouncement.id}`, formData);
        savedAnnouncement = response.data;
      } else {
        const response = await api.post('/branch/announcements', formData);
        savedAnnouncement = response.data;
      }
      ...
      if (pendingFiles.length > 0 && savedAnnouncement?.id) {
        ...
        setUploadProgress(0);
        try {
          await branchAnnouncementAPI.uploadAttachments(
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <CustomSelect value={formData.icon_color} onChange={handleSelectChange('icon_color')} options={[{ value: 'blue', label: 'Blue' }, { value: 'green', label: 'Green' }, { value: 'yellow', label: 'Yellow' }, { value: 'red', label: 'Red' }]} placeholder="Select color" />
                  </div>
                </div>
