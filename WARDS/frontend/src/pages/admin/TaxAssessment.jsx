          ...
          {assessmentForm.tax_type === 'RPT' ? (
            <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
              ...
              <label className="block">
                ...
                <input name="property_type" value={assessmentForm.property_type} onChange={handleAssessmentFormChange} className={`w-full rounded-2xl border px-4 py-3 text-sm ${assessmentValidationErrors.property_type ? 'border-rose-400 bg-rose-50' : 'border-slate-300'}`} />
      ...
      <div className="grid gap-8 xl:grid-cols-[1.05fr,0.95fr]">
        ...
        <section className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.05)]">
          <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <input value={assessmentSearch} onChange={(event) => setAssessmentSearch(event.target.value)} placeholder="Search taxpayer, TDN, permit, business" className="w-full max-w-sm rounded-full border border-slate-300 px-4 py-2.5 text-sm" />
          </div>
            ...
            <div key={submission.id} className="rounded-2xl border border-slate-200 bg-[#fbfdff] p-5">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-2xl bg-slate-50 px-4 py-3">
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-900">{submission.created_at ? new Date(submission.created_at).toLocaleString('en-PH') : 'N/A'}</p>
                </div>
                <div className="rounded-2xl bg-slate-50 px-4 py-3">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">Reviewed</p>
                  <p className="mt-2 text-sm font-semibold text-slate-900">{submission.reviewed_at ? new Date(submission.reviewed_at).toLocaleString('en-PH') : 'N/A'}</p>
                </div>
