...
const TRANSACTION_CATEGORY_OPTIONS = ['Real Property Tax', 'Business Tax', 'Miscellaneous Tax', 'Receipt Request Fee'];
...
const Reports = () => {
  ...
  const validateSearchInput = (value) => {
    ...
    if (!/^[A-Za-z_]+$/.test(value)) {
      return 'Search may only contain letters and underscores.';
    }
    return '';
  };
  ...
  const handleApplyFilters = () => {
    ...
    const searchValidationError = validateSearchInput(filters.search);
          ...
          <div className="xl:col-span-3">
            ...
            <input
              ...
              className={`w-full rounded-xl border px-4 py-3 text-sm ${
                searchError ? 'border-red-500 bg-red-50' : 'border-slate-300'
              }`}
            />
            {searchError && (
              <p className="mt-2 text-sm font-semibold text-red-600">{searchError}</p>
            )}
          </div>
