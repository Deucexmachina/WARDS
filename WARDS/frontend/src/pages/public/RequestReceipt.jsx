...
const RequestReceipt = () => {
  ...
  const localizeModuleError = (message) => {
    ...
    const mappings = {
      ...
      'Too many requests. Please wait before submitting another receipt copy request.': language === 'tl'
        ? 'Masyadong maraming request. Pakihintay bago magsumite muli ng request para sa kopya ng resibo.'
        : 'Too many requests. Please wait before submitting another receipt copy request.',
      'You have reached the hourly request limit. Please try again later.': language === 'tl'
        ? 'Naabot mo na ang hourly request limit. Pakisubukang muli mamaya.'
        : 'You have reached the hourly request limit. Please try again later.',
      'You have reached the maximum number of receipt copy requests allowed for today. Please try again tomorrow.': language === 'tl'
        ? 'Naabot mo na ang maximum na bilang ng request para sa kopya ng resibo ngayong araw. Pakisubukang muli bukas.'
        : 'You have reached the maximum number of receipt copy requests allowed for today. Please try again tomorrow.',
    };
    return mappings[message] || message;
  };

  const isRateLimitMessage = (message) => (
    message === 'Too many requests. Please wait before submitting another receipt copy request.'
    || message === 'You have reached the hourly request limit. Please try again later.'
    || message === 'You have reached the maximum number of receipt copy requests allowed for today. Please try again tomorrow.'
  );
  ...
  useEffect(() => {
    ...
    const resumeRequestId = searchParams.get('requestId');
    const shouldResumeExistingRequest = searchParams.get('resume') === '1' && resumeRequestId;

    if (!shouldResumeExistingRequest) {
      setRequestStatus(null);
      setRequestConfirmed(false);
      return;
    }

    receiptAPI.getRequestStatus(resumeRequestId)
      .then((response) => {
  ...
  }, [language, navigate, searchParams]);
              ...
              <div className="mx-auto max-w-6xl space-y-6">
                ...
                {requestStatus.nextAction === 'Wait for Branch Release' ? (
                  <button
                    ...
                    className="w-full rounded-2xl border border-slate-300 bg-slate-100 px-5 py-3 font-semibold text-slate-800 transition hover:bg-slate-200"
                  >
